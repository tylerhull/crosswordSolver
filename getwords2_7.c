...
#define out_path "/home/tyler/Documents/Github/crosswordSolver/word_list7.txt"
...
int main(){
  ...
  for(i = 0; i < DICT_WORD_SIZE; i++) { // Set the loop for the dictionary length
    ...
    if(strlen(buf) == 8) {    // Need to use 8 here because of the newline char.
        for (j=0; j<7; j++){  // Search current word for the ' character
      ...
      buf[7] = 0;   // Replace the ending newline character with a 0.
      ...
      fprintf (output,"word(%s,%c,%c,%c,%c,%c,%c,%c).\n", buf,buf[0],buf[1], buf[2], buf[3], buf[4], buf[5], buf[6]);
