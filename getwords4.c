...
#define out_path "/home/tyler/Documents/Github/crosswordSolver/allword_list23567.txt"
...
int main(){
  ...
  for(i = 0; i < DICT_WORD_SIZE; i++) { // Set the loop for the dictionary length
    ...
    // If the word is length 2,3,5,6,7 then we write to a file.
    // Add one to the length below to account for the newline character
    if (stringLength == 3 || stringLength == 4 || stringLength == 6 || stringLength == 7 || stringLength == 8){
      if(strlen(buf) > 0) {    // If the word is longer than 0 characters...
